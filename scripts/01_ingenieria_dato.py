...
# Variables con nulos: workclass, occupation, native-country → CATEGÓRICAS NOMINALES
#
# Técnicas evaluadas:
#   1. Moda (elegida): variables nominales sin orden lógico; la moda representa
#      la categoría más frecuente de la distribución real del censo.
#   2. Interpolación lineal: adecuada para variables numéricas con tendencia
#      temporal (p.ej. age, hours-per-week si tuviesen nulos); no aplicable aquí.
#   3. KNN imputation: alternativa válida para variables continuas; mayor coste
#      computacional; no aporta ventaja sobre moda en variables nominales.
#   4. Eliminación de filas: descartada — tasas < 5%; eliminar 4.262 filas
#      introduciría sesgo de selección.
#
# Para variables numéricas con nulos se aplicaría:
#   df[cols_num].interpolate(method='linear', limit_direction='both', inplace=True)
# ──────────────────────────────────────────────────────────────────────────────

# Verificar si hay nulos en variables numéricas (para documentar la decisión)
COLS_NUM = ['age', 'fnlwgt', 'education-num', 'capital-gain', 'capital-loss', 'hours-per-week']
nulos_num = df[COLS_NUM].isnull().sum()
print("  Variables numéricas con nulos:")
print(f"  {nulos_num[nulos_num>0].to_dict() if nulos_num.sum()>0 else '  → Ninguna (0 nulos en variables numéricas)'}")
print("  → Para variables numéricas se aplicaría interpolación lineal.")
print("  → Los nulos están exclusivamente en variables categóricas → moda.\n")
...
for col in COLS_NULOS:
    ...
    print(f"  ✓ {col}: {n_nulos:,} nulos ({pct:.2f}%) imputados con moda='{moda}'")
...
# =============================================================================
print("\n[ESTADÍSTICA DESCRIPTIVA — Dataset Limpio]")
print("-" * 40)

COLS_DESC = ['age', 'education-num', 'hours-per-week', 'capital-gain', 'capital-loss', 'income_binary']
desc = df_clean[COLS_DESC].describe().round(3)
print(desc.to_string())

# Correlación con la variable objetivo
print("\n  Correlaciones con income_binary:")
corr_target = df_clean[COLS_DESC].corr()['income_binary'].drop('income_binary').sort_values(ascending=False)
for col, val in corr_target.items():
    print(f"    {col:20s}: r = {val:.4f}")

# Desbalance de clases
print(f"\n  Distribución de income_binary:")
vc = df_clean['income_binary'].value_counts(normalize=True)
print(f"    ≤50K (0): {vc[0]*100:.1f}%  |  >50K (1): {vc[1]*100:.1f}%")
print(f"    Ratio desbalanceo: {vc[0]/vc[1]:.2f}:1")
...
print(f"  │ Nulos en categóricas (workclass, occ, nc): 4.262   │")
print(f"  │ Técnica imputación: moda (categóricas)             │")
print(f"  │ Técnica alternativa: interpolación (numéricas)     │")
print(f"  │ Outliers tratados: IQR capping (4 variables)       │")
print(f"  │ Variables nuevas (feature engineering):     5      │")
print(f"  │ Registros finales:                       48.651     │")
print(f"  │ Variables finales:                           20     │")
print(f"  └─────────────────────────────────────────────────────┘")
